...
function loadAvailabilityItems() {
  ...
  if (Object.keys(_availBookings).length === 0 && _inventoryItems.length > 0) {
    var today = new Date();
    var firstItem = _inventoryItems[0].id;
    for (var i = 3; i < 7; i++) {
      var d = new Date(today.getFullYear(), today.getMonth(), today.getDate() + i);
      var key = firstItem + '-' + formatDateKey(d);
      _availBookings[key] = 'booked';
    }
    for (var j = 12; j < 15; j++) {
      var d2 = new Date(today.getFullYear(), today.getMonth(), today.getDate() + j);
      var key2 = firstItem + '-' + formatDateKey(d2);
      _availBookings[key2] = 'blocked';
    }
  }
}
