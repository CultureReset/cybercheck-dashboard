...
function loadStaff() {
  ...
  if (_staffMembers.length === 0) {
    _staffMembers = [
      { id: 1, name: 'Maria Rodriguez', role: 'Head Chef', bio: 'Classically trained with 15 years of experience in French and Italian cuisine.', photo: '' },
      { id: 2, name: 'James Chen', role: 'Sous Chef', bio: 'Specializes in seasonal farm-to-table dishes.', photo: '' },
      { id: 3, name: 'Sarah Mitchell', role: 'General Manager', bio: 'Leading our team since 2019 with a passion for exceptional service.', photo: '' }
    ];
    _staffIdCounter = 3;
  }
...
function saveStaff() {
  ...
  if (photoInput.files && photoInput.files[0]) {
    ...
    reader.onload = function(e) {
      finishSaveStaff(id, name, role, bio, e.target.result);
    };
    reader.readAsDataURL(photoInput.files[0]);
  } else {
    var existingPhoto = '';
    if (id) {
      var existing = _staffMembers.find(function(m) { return m.id === parseInt(id); });
...
function finishSaveStaff(id, name, role, bio, photo) {
  if (id) {
    var member = _staffMembers.find(function(m) { return m.id === parseInt(id); });
    if (member) {
      member.name = name;
      member.role = role;
      member.bio = bio;
...
function deleteStaff(id) {
  ...
  _staffMembers = _staffMembers.filter(function(m) { return m.id !== id; });
  renderStaff();
